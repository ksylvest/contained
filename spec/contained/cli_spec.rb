...
RSpec.describe Contained::CLI do
  ...
  describe "#parse" do
    ...
    context "with an 'init' command" do
      let(:argv) { ["init"] }

      it "calls Contained::CLI::Init" do
        allow(Contained::CLI::Init).to receive(:handle!)
        parse
        expect(Contained::CLI::Init).to have_received(:handle!).with(stdin:, stdout:, argv:)
      end
    end

    context "with a 'setup' command" do
      let(:argv) { ["setup"] }

      it "calls Contained::CLI::Init" do
        allow(Contained::CLI::Setup).to receive(:handle!)
        parse
        expect(Contained::CLI::Setup).to have_received(:handle!).with(stdin:, stdout:, argv:)
      end
    end
